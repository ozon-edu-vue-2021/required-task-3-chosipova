<template>
  <div class="menu">
    ...
    <div class="content">
          ...
          <div v-if="legend.length > 0" class="legend__items">
            <Draggable v-model="legend">
              <LegendItem
                ...
                :color="item.color"
                :text="item.text"
                :counter="tablesCount(item.group_id)"
                class="legend__item"
              />
            </Draggable>
          </div>
      ...
      <div v-else class="profile" v-click-outside="closeProfile">
...
export default {
  ...
  methods: {
        ...
        datasets: [
          {
            ...
            data: this.legend.map((legendItem) =>
              this.tablesCount(legendItem.group_id)
            ),
          },
        ],
      };
    ...
    tablesCount(id) {
      return tables.filter((item) => item.group_id == id).length;
    },
  },
  directives: {
    ClickOutside,
